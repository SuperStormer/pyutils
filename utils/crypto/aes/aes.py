...
def aes_ecb(s, key, mode, *, pad=True):
...
def aes_cbc(s, key, iv, mode):
	...
	for block in blocks:
		if mode == "encrypt":
			transformed = aes_ecb(xor(block, prev), key, mode, pad=False)
			prev = transformed
...
def aes_ctr_keystream(key, nonce):
	for counter in itertools.count():
		yield from aes_ecb(struct.pack("<QQ", nonce, counter), key, "encrypt", pad=False)
