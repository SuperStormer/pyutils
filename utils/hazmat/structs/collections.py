...
class setentry(Struct):  #pylint: disable=invalid-name
...
class PySetObject(Struct):
	...
	_fields_ = [
		...
		("finger", ctypes.c_ssize_t), ("smalltable", (setentry * MINSIZE)),
...
class rangeobject(Struct):  #pylint: disable=invalid-name
	_fields_ = [
		("ob_base", PyObject), ("_start", PyObject_p), ("_stop", PyObject_p), ("_step", PyObject_p),
		("_length", PyObject_p)
	]
